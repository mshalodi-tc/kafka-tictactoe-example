...
def main(args):
  ...
  consumer.subscribe([player2_topic])

  print("player2 started consuming...")
  ...
  while True:
    try:
      ...
      if message is not None:
        # print(f"player2 consumed message with grid={message.grid}\n")
        available_slots = []
        for i in range(len(message.grid)):
          if message.grid[i] == '0':
            available_slots.append(i)
        random_index = random.randint(0, len(available_slots))
        message.grid = message.grid[:available_slots[random_index]] + '2' + message.grid[available_slots[random_index] + 1:]
...
if __name__ == '__main__':
  ...
  parser.add_argument('-g', dest="group", default="player2", help="Consumer group")
